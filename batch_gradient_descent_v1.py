...
# ======================================= #
# def gradient(param_vector):
#     """
#     param_vector: vector of parameters
#     return: gradient vector evaluated at
#              the given set of parameters
#     """
#     error = (y - np.dot(X, param_vector)) # prediction error
#     g = (-1/N) * np.dot(np.transpose(X), error) # gradient vector
#     return g
...
#          the parameter vector supplied
#     """
#     y_ = np.dot(X, param_vector) # predicted by the model 
#     cost = np.sum(np.square(y - y_))/(2*N)
#     return cost

# param_current = np.random.randint(1, 8, size=4) # parameters are initialized by uniformly distributed random values between (0, 1)
# cost_current = cost_fn(param_current)

# prediction = np.dot(X, param_current)
# prediction_error = (y - prediction)

# param_new = param_current - (learning_rate * gradient(param_current))
# cost_new = cost_fn(param_new)
# cost_diff = abs(cost_new - cost_current)

# step = 1 # steps required to converge, initialized as 1
# print("Step:{}\tUpdated cost:{}\tCost difference:{}\tUpdated parameters:{}".format(step, cost_fn(param_new), cost_diff, param_new))
...
#     cost_new = cost_fn(param_new)
#     cost_diff = abs(cost_new - cost_current) 
#     converged = bool(cost_diff < 0.0000000001)
#     if step % 50 == 0:
#         # prints the summary after each 50th step
#         print("Step:{}\tUpdated cost:{:0.4f}\tCost difference:{}\tUpdated parameters:{}".format(step, cost_fn(param_new), cost_diff, param_new))
#     step = step + 1