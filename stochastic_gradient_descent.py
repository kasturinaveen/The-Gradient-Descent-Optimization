...
model using "Stochastic Gradient Descent" (SGD) cost (or loss) minimization
algorithm. First we simulate the dummy data specifying the parameters and then
we build our own code for BGD to find the estimates of the paremeters.
...
def stochastic_gradient_descent(init, learning_rate, data_train,
...
def main():
    ...
    precision = 0.000001
    try:
        stochastic_gradient_descent(p, learning_rate, data_train,
                                    target_train, precision)
    except TypeError:
